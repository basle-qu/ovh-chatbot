...
const mobile = require("../platforms/mobile/mobile");
...
module.exports = () => {
  ...
  function onGet (req, res) {
    ...
    return mobile.getHistory(res, nichandle, deviceid)
      .then((result) => {
        if (!result.length) {
          mobile.send(null, nichandle, deviceid, translator("welcomeWeb", locale, nichandle));
        }
  ...
  function onPost (req, res) {
    ...
    if (!deviceid) {
      return res.status(403).json(new Error("Missing Device ID"));
    }
    ...
    // send nichandle or deviceid to apiai
    if (nichandle) {
      id = nichandle;
    } else {
      id = deviceid;
    ...
    return mobile
      .pushToHistory(nichandle, deviceid, userMsg)
      .then(() => {
        if (type === "message") {
          return messageReceived(res, id, message, locale);
        } else if (type === "postback") {
          return postbackReceived(res, id, message, locale);
        }
        throw new Error(`unknown type, ${type}`);
      })
      .then((resultLocal) => {
        result = resultLocal;
        return sendResponses(res, nichandle, deviceid, result.responses, locale);
      })
  ...
  function sendResponses (res, nichandle, deviceid, responses, result, locale) {
    ...
    if (promises.length) {
      Bluebird.mapSeries(promises, (response) =>
        Bluebird.resolve(response)
          .then((resp) => sendResponses(res, nichandle, deviceid, resp, result, locale)));
    }

    return Bluebird.all(generics.map((uGeneric) => mobile.send(res, nichandle, deviceid, uGeneric)));
  }
